...
for i in range(1, 13):
    urldum = "https://www.cvedetails.com/vulnerability-list/year-1999/month-{}"
    url = urldum.format(i)
...
conn = sqlite3.connect('cvedata1999test')
c = conn.cursor()
def create_table():
    c.execute('CREATE TABLE IF NOT EXISTS cvedata1999test(Cve_id INT,Description TEXT,Publish_Date  INT,Update_Date INT)')
...
try:
    for i in range(len(Cve_id)):
        ...
        III = Publish_Date[i]
        IV = Update_Date[i]
        c.execute("INSERT INTO cvedata1999test(Cve_id, Description, Publish_Date, Update_Date) VALUES  (?,?,?,?) ", (I, II, III, IV))
        conn.commit()
